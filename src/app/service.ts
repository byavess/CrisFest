...
import { INossoAcervo } from './interfaces/INossoAcervo';
...
export class Service {
 ...
 getVeiculos(): Observable<INossoAcervo[]> {
    return this.http.get<INossoAcervo[]>('assets/data/veiculos.json');
  }
  getVeiculoById(id: number, withAuth: boolean = false) {
  //getVeiculoById(id: number, withAuth: boolean = false): Observable<INossoAcervo> {
    let options = {};
    if (withAuth) {
    ...
    //return this.http.get<INossoAcervo>(`${this.apiUrl}/${id}`, options);
  }
  ...
  // Busca todas as marcas disponíveis *get meninas
    getAllMarcas(): Observable<IMeninas[]> {
return this.http.get<[]>('assets/images/marcas.jpeg');
  }
  ...
  getModelos(marcaId?: number): Observable<IMeninos[]> {

    const url = marcaId
    //  ? `${environment.apiBaseUrl}/modelos?marcaId=${marcaId}`
return this.http.get<[]>('assets/images/marcas.jpeg');
      //: `${environment.apiBaseUrl}/modelos`;
    //return this.http.get<IMeninos[]>(url);
  }
   ...
   */
 // openWhatsApp(veiculo?: INossoAcervo, whatsappNumber: string = '61984321908', customMessage?: string): void {
   // let message: string;
...
Poderia me enviar mais informações?`;
   ...
   */
    getVeiculosVendidos() {
 // getVeiculosVendidos(): Observable<INossoAcervo[]> {
    return this.http.get<[]>(`${this}?vendido=true`);
  }
